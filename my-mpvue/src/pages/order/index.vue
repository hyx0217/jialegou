<template>
  <div>
    <van-tabs :active="active">
      <van-tab title="全部">
        <p v-for="(item,index) in allOrder" :key="index" style="margin-top:20rpx">
          <van-card num="2" :price="item.price" desc="描述信息" :title="item.name" :thumb="item.img"
            ...
            <view slot="footer">
              <div v-show="!item.pay">
                <van-button size="small" @click="delOrder(item._id)">取消订单</van-button>
                <van-button size="small" @click="cfmOrder(item._id)">确认付款</van-button>
              </div>
              <div v-show="(item.pay && !item.send) ">
                <van-button size="small">提醒发货</van-button>
              </div>
              <div v-show="(item.send && !item.receive)">
                <van-button v-if="!item.receive" size="small" @click="receive(item._id)">确认收货</van-button>
                <p v-else>已收货</p>
              </div>
              <div v-show="item.receive">
                <van-button v-if="!item.comment" size="small" @click="goComment(item._id,item.G_id)">评价</van-button>
                <p v-else>已评价</p>
              </div>
            </view>
          </van-card>
      ...
      <van-dialog use-slot :show="show" show-cancel-button @cancel="show=false" @confirm="pushComment()">
        <van-cell-group>
          <van-field :value="comment " label="评价" type="textarea" placeholder="请输入评价" autosize :border="false"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pushComment() {
        ...
        this.$fly
          ...
          .then(res => {
            //评论完修改评论状态
            this.$fly
              .put(`${this.baseUrl}/order/${this.O_id}`, {
                comment: true
              })
              .then(res => {
                this.getAll();
      ...
      getAll() {
        this.$fly
          .post(`${this.baseUrl}/order/getorder`, {
            U_id: store.state.userId
          })
          .then(res => {
      ...
      //确认订单
      cfmOrder(id) {
        this.$fly.put(`${this.baseUrl}/order/${id}`, {
          pay: true
        }).then(res => {
          this.getAll();
        });
